...
FROM Products --This query will return the ProductName column but display it as Name in the result set

--2. Write a query that uses an alias to rename the Customers table as Client for easier reference.
SELECT *
FROM Customers AS Client --This query will refer to the Customers table as Client for easier reference throughout the query.
...
FROM Products
--This query will display the ProductName, Price, and a new column named PriceCategory. The value in PriceCategory will be 'High' if the price is greater than 1000 and 'Low' if the price is 1000 or less.
...
FROM Products_Discounted
--StockQuantity > 100 is the condition.
--'Yes' is returned if the condition is true.
--'No' is returned if the condition is false.
--The result is displayed in a new column named StockStatus
...
SELECT ProductName
FROM OutOfStock
--The UNION operator will combine the results from both queries, removing any duplicate ProductName values
...
SELECT ProductName
FROM Products_Discounted
--This query will return all ProductName values that are in the Products table but not in the Products_Discounted table.
...
FROM Products
--If Price > 1000, the new column PriceCategory will display 'Expensive'.
--If Price <= 1000, the new column will display 'Affordable'

--11. Write a query to find employees in the Employees table who have either Age < 25 or Salary > 60000.
SELECT *
FROM Employees
WHERE Age < 25 OR Salary > 60000
--This query retrieves all columns for employees who either have an Age less than 25 or a Salary greater than 60000. The OR operator is used to combine the two conditions
...
--UPDATE Employees
--SET Salary = 
--    IF(Department = 'HR' OR EmployeeID = 5, Salary * 1.10, Salary)
--WHERE Department = 'HR' OR EmployeeID = 5;

--SQL Server doesn't allow the use of the IF statement directly inside the UPDATE query like that. SQL Server requires a different approach for conditional updates. We should use a CASE statement instead of IF in the UPDATE query
...
SELECT SaleAmount,
       ...
       END AS SaleTier
FROM Sales
--This query will return the SaleAmount and the corresponding SaleTier based on the conditions provided
...
SELECT CustomerID
FROM Invoices;
--The EXCEPT operator returns the customer IDs that are in the Orders table but not in the Invoices table, meaning those customers placed orders but don't have a corresponding record in the Invoices table
